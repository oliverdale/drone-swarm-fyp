...
def init_socket():
    ...
    socket.bind("tcp://*:{}".format(flight_parameters['COMMON']['TARGET_POS_PORT'])) # Must match the one in mavros_offboard_posctl.py and rx.py or client_gps
    return socket
    ...
    while True:
        ...
        if splitline[0] == GPGGA:
            ...
            try:
                ...
                coord = GPSCoord.from_nmea(latitude_nmea, longitude_nmea, TARGETALT)
            except ValueError:
                # Output from GPS occasionally erroneous
                print("Invalid GPS line:", line)
                continue
            
            if abs(time.time()-last_time)> LOOP_PERIOD_S:
                last_time = time.time()
                ...
                socket.send(message)
                log_file.write("{}, {}, {}, {}m\n".format(time_sample, coord.lat, coord.long, coord.alt))
