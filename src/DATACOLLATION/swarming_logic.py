...
from datacollation import logCOORD_desired
""" new lines 39, 43, 47, 51, 55: these log into the intelNUC"""
...
def update_loc(target, drone_num, drone_pos):
    ...
    if target.lat == -1 or target.long == -1:
        ...
        if drone_num == 0:
            ...
            logCOORD_desired(pos, drone_num) # logs mothership
        elif drone_num == 1:
            pos = target.add_x_offset(-5)
            pos = pos.add_y_offset(5)
            logCOORD_desired(pos, drone_num) # logs drone 1
        elif drone_num == 2:
            pos = target.add_x_offset(5)
            pos = pos.add_y_offset(5)
            logCOORD_desired(pos, drone_num)  # logs drone 2
        elif drone_num == 3:
            pos = target.add_x_offset(-5)
            pos = pos.add_y_offset(-5)
            logCOORD_desired(pos, drone_num)  # logs drone 3
        elif drone_num == 4:
            pos = target.add_x_offset(5)
            pos = pos.add_y_offset(-5)
            logCOORD_desired(pos, drone_num)  # logs drone 4
    return pos
