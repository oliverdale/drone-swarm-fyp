...
class PosFilter:
    ...
    def __init__(self, drones_i, z_std_drone, z_std_range, initial_target, dt=None, initial_target_uncertanty=0.1):
        """
...
def generate_ranges(tx, rx, tar):
    ...
    target = GPSCoord(tar[0], tar[1], tar[2])
...
def main ():
    ...
    filt = PosFilter(drones_i, 0.5, 0.5, target_i, ts)
    ...
    cart_p = filt.get_swarm_item_pos(5)
    pos_plot = plt.figure(3)
    ax = pos_plot.add_subplot(111, projection='3d')

    ax.plot(cart_p[:, 0], cart_p[:, 1], cart_p[:, 2], 'magenta', label = 'filter') #estimated
