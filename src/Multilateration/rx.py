...
class ReceiverUAV:
    ...
    def calculate_range(self, target_coords):
        ...
        """
        rx_coords_3D = GPSCoord3D(self.get_rx_coords().lat, self.get_rx_coords().long, RX_ALTITUDE)
        tx_coords_3D = GPSCoord3D(self.tx_coords.lat, self.tx_coords.long, TX_ALTITUDE)
        target_coords_3D = GPSCoord3D(target_coords.lat, target_coords.long, TARGET_ALTITUDE)

        return (rx_coords_3D.distance(target_coords_3D) +
                target_coords_3D.distance(tx_coords_3D))
