...
# Address of device running server_gps.py
...
class MultiMavrosOffboardPosctl(MultiMavrosCommon):
    ...
    def take_off(self, auto_arm=False):
        ...
        MSL = self.altitude.amsl
        self.default_altitude = FLYING_ALTITUDE + MSL
    ...
    def convert_global_to_local(self, lat, lon, alt):
        ...
        """
        current_coords = GPSCoord(self.global_position.latitude, self.global_position.longitude)
        setpoint_coords = GPSCoord(lat, lon)
    ...
    def reach_position(self, lat, lon, alt=None):
        ...
        rospy.loginfo(
            "attempting to reach position x: {:.6f}, y: {:.6f}, z: {:.6f} | current position x: {:.6f}, y: {:.6f}, z: {:.6f}".
                format(lat, lon, alt,
    ...
    def get_current_position(self):
        ...
        return GPSCoord(self.global_position.latitude, self.global_position.longitude)
...
class Controller(object):
    ...
    def run_posctl(self):
        ...
        self.setpoint_alt = FLYING_ALTITUDE
        rate = rospy.Rate(5)
