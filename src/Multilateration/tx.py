...
class TransmitterUAV:
    ...
    def perform_multilateration(self, prev_est):
        ...
        result = multilateration_v2.est_target_pos(
            self.get_tx_coords(), rx_positions, ranges, prev_est)
        return result
    ...
    def run(self):
        ...
        target_coords = self.get_tx_coords()
        ...
        while True:
            ...
            target_coords = self.perform_multilateration(target_coords) #Returns target position in 3D coordinates
            target_coords = GPSCoord(target_coords.lat, target_coords.long) # Convert back to 2D coordinates
            self.target_positions.append(target_coords)

            print("Multilateration error {:.2}".format(
